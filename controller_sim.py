import numpy as np
import matplotlib.pyplot as plt

from backstepping_controller import backstepping_controller
from SL_planner import StraightLinePathPlanner


# =========================
# Simulation settings
# =========================
dt = 0.01
T = 1000
N = int(T / dt)
noise_power = 10
sigma = np.sqrt(noise_power)

# =========================
# Initial conditions
# =========================
eta = np.array([-10.0, 10.0, 0.0])   # [x, y, psi]
nu = np.array([0.0, 0.0, 0.0])
b = np.zeros(3)                    # bias

s = 0.0


# =========================
# Instantiate system
# =========================
planner = StraightLinePathPlanner(
    p0=[0, 0],
    p1=[20, 40],
    psi_d = 90.0,   # endre her
    U_ref = 0.5,   # endre her
    mu = 0.01       # endre her
)

controller = backstepping_controller()


# =========================
# Logging
# =========================
eta_log = np.zeros((N, 3))
eta_d_log = np.zeros((N, 3))
nu_log = np.zeros((N, 3))
tau_log = np.zeros((N, 3))
s_log = np.zeros(N)
z1_log = np.zeros((N, 3))
z2_log = np.zeros((N, 3))

# =========================
# Simulation loop
# =========================

projection_done = False
s_dot_tol = 1e-3

for i in range(N):

    w = sigma * np.random.randn(3)

    # Path planner
    eta_d, eta_d_s, eta_d_ss, v_s, v_s_s, s_dot = planner.get_signals(eta, s)

    if s >= 1.0:
        s_dot = 0.0
        v_s = 0.0
        #v_s_s = 0.0

    if not projection_done:
        if abs(s_dot) < s_dot_tol:
            projection_done = True
            tau = controller.step(
                eta, nu, b,
                eta_d, eta_d_s, eta_d_ss,
                v_s, v_s_s, s_dot
            )
        else:
            tau = np.zeros(3)
    else:
        # Controller
        tau = controller.step(
            eta, nu, b,
            eta_d, eta_d_s, eta_d_ss,
            v_s, v_s_s, s_dot
        )

    # Rotation matrix
    psi = eta[2]
    R = np.array([
        [np.cos(psi), -np.sin(psi), 0],
        [np.sin(psi),  np.cos(psi), 0],
        [0, 0, 1]
    ])

    z1 = R.T @ (eta - eta_d)

    # alpha1
    alpha1 = -controller.K1 @ z1 + (R.T @ eta_d_s) * v_s

    # z2
    z2 = nu - alpha1

    # log
    z1_log[i] = z1
    z2_log[i] = z2

    # Dynamics
    nu_dot = np.linalg.inv(controller.M) @ (tau - controller.D @ nu + b + w)
    eta_dot = R @ nu

    # Integrate (Euler)
    nu = nu + dt * nu_dot
    eta = eta + dt * eta_dot
    s = s + dt * s_dot
    s = np.clip(s, 0.0, 1.0)

    # Log
    eta_log[i] = eta
    eta_d_log[i] = eta_d
    nu_log[i] = nu
    tau_log[i] = tau
    s_log[i] = s


# =========================
# Plot position
# =========================
plt.figure(figsize=(8,6))

# Path (linjen)
p0 = planner.p0
p1 = planner.p1
plt.plot([p0[0], p1[0]], [p0[1], p1[1]], 'k--', label='Path')
plt.plot(planner.p0[0], planner.p0[1], "ko", label="Start path")
plt.plot(planner.p1[0], planner.p1[1], "ko", label="End path")

# Vessel trajectory
plt.plot(eta_log[:,0], eta_log[:,1], label="Vessel")

# Startpunkt
plt.scatter(eta_log[0,0], eta_log[0,1], c='green', s=80, label="Start")

# Sluttpunkt
plt.scatter(eta_log[-1,0], eta_log[-1,1], c='red', s=80, label="End")

# =========================
# Heading piler (skip)
# =========================
skip = 1000  # hvor ofte vi tegner pil

for i in range(0, N, skip):
    x = eta_log[i,0]
    y = eta_log[i,1]
    psi = eta_log[i,2]

    dx = np.cos(psi)
    dy = np.sin(psi)

    plt.arrow(x, y, dx, dy,
              head_width=0.3,
              head_length=0.5,
              fc='blue', ec='blue')

# =========================
# Reference heading (path retning)
# =========================
psi_d = planner.psi_d

x_mid = (p0[0] + p1[0]) / 2
y_mid = (p0[1] + p1[1]) / 2

plt.arrow(x_mid, y_mid,
          np.cos(psi_d), np.sin(psi_d),
          head_width=0.4, head_length=0.7,
          fc='black', ec='black',
          label="Path direction")

# =========================

t = np.linspace(0, T, N)

plt.xlabel("x")
plt.ylabel("y")
plt.axis("equal")
plt.grid()
plt.legend()
plt.title("Path Following with Heading")
plt.show()

plt.figure()

plt.subplot(3,1,1)
plt.plot(t, z1_log[:,0], label="z1_x")
plt.plot(t, z2_log[:,0], label="z2_u")
plt.legend()
plt.grid()

plt.subplot(3,1,2)
plt.plot(t, z1_log[:,1], label="z1_y")
plt.plot(t, z2_log[:,1], label="z2_v")
plt.legend()
plt.grid()

plt.subplot(3,1,3)
plt.plot(t, z1_log[:,2], label="z1_psi")
plt.plot(t, z2_log[:,2], label="z2_r")
plt.legend()
plt.grid()

plt.xlabel("Time [s]")
plt.suptitle("Error components")
plt.show()